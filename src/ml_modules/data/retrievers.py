...
class AlphaFold_Retriever: # The Golden AF Retriever
    ...
    def batch_retrieve(self, accessions, item_list=['pdb'], retry=False):
...
class UniProt_Retriever:
    ...
    def __init__(self):
        ...
        os.makedirs(self.uniprot_dir, exist_ok=True)
        self.kb_dir = osp.join(self.uniprot_dir, 'kb')
    ...
    def path_to_data(self, accession):
        return osp.join(self.kb_dir, f'{accession}.txt')
...
class PDB_Retriever:
    ...
    def __init__(self):
        ...
        self.pdb_dir = osp.join(external_dir, 'PDB')
    ...
    def path_to_file(self, accession):
        ...
        if accession.endswith(self.suffix):
            return osp.join(self.prody_cache_dir, f'{accession}.{extension}')
        else:
            return osp.join(
                self.prody_cache_dir,
                f'{self.modify_accession(accession)}.{extension}'
            )
...
if __name__ == '__main__':
    ...
    pdbID_file = '../../../notebooks/20250527-2 homo-multimer dataset - from DeepSTABP dataset (copy of 20240817 from ai-Tm_prediction)/OUT-4.meta-homomultimers.csv'
