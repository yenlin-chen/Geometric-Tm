...
def summarize_couplings(
    ref_accessions, stats_dir, structure_source='AlphaFold'
    ...
    if structure_source == 'AlphaFold':
        ...
        struct_retriever = PDB_Retriever()
    else:
...
if __name__ == '__main__':
    ...
    ref_accessions = np.loadtxt(
        f'{stats_dir}/accessions - {reference_set}.txt',
        dtype=np.str_,
    )

    summarize_couplings(
        ref_accessions, structure_source='AlphaFold', stats_dir=stats_dir
    )
