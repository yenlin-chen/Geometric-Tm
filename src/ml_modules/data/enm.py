...
import os, shutil, subprocess, platform, json, warnings
import numpy as np
...
if __name__ == '__main__':
    ...
    pdb_ids = np.loadtxt(
        f'{stats_dir}/metadata - {set_name}.csv',
        delimiter=',',
        usecols=0,
        dtype=np.str_
    )[:5]
    mod_pdb_ids = [f'{pdb_id}-PDB' for pdb_id in pdb_ids]
    pdb_path_list = [
        f'/mnt/hdd/yenlin/projects/Geometric-Tm/data/external/PDB/cache/{mod_pdb_id}.pdb'
        for mod_pdb_id in mod_pdb_ids
    ]
    ...
    successful, failed = tnm_computer.batch_run(
        mod_pdb_ids, pdb_path_list, retry=False
    )
