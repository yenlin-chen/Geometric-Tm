...
self_dir = os.getcwd()
root_dir = os.path.normpath(self_dir + '/..' * 4)
package_dir = os.path.join(root_dir, 'src')
sys.path.append(package_dir)

from ml_modules.training.model_arch import SimplifiedMultiGCN, mGCN
...
def main():
    ...
    if model_type == 'mGCN':
        graph_dims = edge_types_to_use  # mGCN only
        mgcn_hidden_channels = 256  # mGCN only
        dim_node_hidden_ls = [32]  # mGCN only

    elif model_type == 'SimplifiedMultiGCN':
        dim_node_hidden_dict = {  # SimplifiedMultiGCN only
    ...
    tv_meta_file = os.path.join(exp_root_dir, f'metadata - {tv_set_name}.csv')
    test_meta_file = os.path.join(exp_root_dir, f'metadata - {test_set_name}.csv')
    ...
    for fold_idx in range(n_folds):
        ...
        ################################################################
        if model_type == 'mGCN':
            model = mGCN(  # mGCN only
                # FEATURE SELECTION
                graph_dims=graph_dims,  # mGCN only
                use_ogt=use_ogt,
                use_pi=use_pi,
                # GRAPH CONVOLUTION SETUP
                node_feat_name='x',
                node_feat_size=1024,
                # gnn_type='gcn',  # SimplifiedMultiGCN only
                # gat_atten_heads=None,  # SimplifiedMultiGCN only
                # dim_node_hidden_dict=dim_node_hidden_dict,  # SimplifiedMultiGCN only
                dim_node_hidden_ls=dim_node_hidden_ls,  # mGCN only
                mgcn_hidden_channels=mgcn_hidden_channels,  # mGCN only
                conv_norm=True,
            ...
            model = SimplifiedMultiGCN(  # SimplifiedMultiGCN only
                # FEATURE SELECTION
                # graph_dims=graph_dims,  # mGCN only
                use_ogt=use_ogt,
                use_pi=use_pi,
        ...
        valid_idx = torch.tensor(
            [np.where(tv_set.processable_accessions == a)[0][0] for a in valid_acc]
        )
